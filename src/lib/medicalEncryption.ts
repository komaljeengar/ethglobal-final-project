...
export class MedicalEncryptionService {
  ...
  static async uploadEncryptedDocument(
    ...
    try {
      ...
      const {
        ...
        iv: ivArray,
        size,
      } = await this.encryptFile(file, aesKey, iv);

      // 3. Upload encrypted file to IPFS
      const encryptedFileCID = await this.uploadToIPFS(
        encryptedData,
      ...
      console.log(
        `Document encrypted and NFT minted with token ID: ${result.tokenId}`
      );
   ...
   */
  static async downloadAndDecryptDocument(
    tokenId: number,
   ...
   */
  static async mintEncryptedNFT(nftData: {
    patientAddress: string;
    metadataCID: string;
    wrappedKey: number[];
    documentType: string;
  ...
  static async storePrivateKey(privateKey: CryptoKey, keyId: string) {
    ...
    console.warn(
      "Private key stored in localStorage - use secure storage in production!"
    );
